(ns parallel-colt-matrix.core-test
  ...
  (:require [parallel-colt-matrix.matrix :as pcm])
...
(deftest vector-dimensionality-test
  (is (== 2 (pcm/vector-dimensionality [[1 2 3]])))
  (is (== 3 (pcm/vector-dimensionality [[[1] [2] [3] [4]]]))))
...
(deftest MatrixSubComponents
  (is (= (main-diagonal (pcm/get-matrix [[1 2]
                                        [3 4]])) '(1.0 4.0)))
  (is (= (main-diagonal (pcm/get-matrix [[1 2 3]
                                        [4 5 6]])) '(1.0 5.0)))
  (is (= (main-diagonal (pcm/get-matrix [[1 2]
                                        [3 4]
                                        [5 6]])) '(1.0 4.0)))
...
(deftest MatrixScaling-test
  (is (= (scale (pcm/get-matrix [[1 2] [3 4]]) 3) (pcm/get-matrix [[3 6] [9 12]])))
  (is (= (scale (pcm/get-matrix [[1 2] [3 4]]) -1) (pcm/get-matrix [[-1 -2] [-3 -4]])))
  (is (= (scale (pcm/get-matrix [[1 2] [3 4]]) 1.5) (pcm/get-matrix [[1.5 3] [4.5 6]]))))

(deftest MatrixMutableScaling-test
  (let [m (pcm/get-matrix [[1 2] [3 4]])]
    (is (= (pcm/get-matrix [[2 4] [6 8]]) (scale! m 2)))
    (is (= (pcm/get-matrix [[2 4] [6 8]]) m))
    (is (= (pcm/get-matrix [[1 2] [3 4]]) (scale! m 1/2)))
    (is (= (pcm/get-matrix [[1 2] [3 4]]) m))
    (is (= (scale m 3) (scale! m 3)) "Testing consistency between scale and scale!")
    (is (not= (scale (pcm/get-matrix [[1 2] [3 4]]) 2) (scale m 2)) "m is now different from the start")
    (is (not= (pcm/get-matrix [[1 2] [3 4]]) m))))

(deftest Reshaping-test
  (let [m (pcm/get-matrix [[1 2 3] [4 5 6]])]
    (is (= (reshape m [2 2]) (pcm/get-matrix [[1 2]
                                             [3 4]])))
    (is (= (reshape m [2 3]) (pcm/get-matrix [[1 2]
                                             [3 4]
                                             [5 6]])))
...
(deftest DoubleArrayOutput-test
  (let [m (pcm/get-matrix [[1 2 3] [4 5 6]])
    ...
    (is (= m (pcm/get-matrix [[1 42 3] [4 5 6]])) "Check if it really change the underneath array")
    (aset (to-double-array m) 2 53.0)
    (is (not= m (pcm/get-matrix [[1 42 53] [4 5 6]])) "Check correct use of TO-double-array")
    (is (= m (pcm/get-matrix [[1 42 3] [4 5 6]])))))

;; (deftest compilance-testing
;;   (clojure.core.matrix.compliance-tester/compliance-test (pcm/get-matrix [[1 2] [3 4]])))
